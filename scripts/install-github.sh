#!/bin/bash
#
# ClaudeCodeApiConfigManager (ccm) 一键安装脚本 (GitHub 源)
# 适用于 Linux 和 macOS
#
# 用法: curl -fsSL https://raw.githubusercontent.com/morilong/ClaudeCodeApiConfigManager/master/scripts/install-github.sh | bash
#
...
get_latest_version() {
    ...
    VERSION=$(curl -fsSL https://api.github.com/repos/morilong/ClaudeCodeApiConfigManager/releases/latest | grep -o '"tag_name": *"[^"]*"' | cut -d'"' -f4 | sed 's/^v//')

    if [ -z "$VERSION" ]; then
        print_error "无法从 GitHub API 获取版本信息"
        echo ""
        echo "请浏览器打开："
...
download_and_extract() {
    ...
    DOWNLOAD_URL="https://github.com/morilong/ClaudeCodeApiConfigManager/releases/download/v${VERSION}/${FILENAME}"
